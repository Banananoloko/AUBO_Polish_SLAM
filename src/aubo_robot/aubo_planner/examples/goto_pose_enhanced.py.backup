...
# -*- coding: utf-8 -*-
"""
goto_pose_enhanced.py - 增强版交互式坐标输入，带彩色UI和详细状态
...
def print_header():
    ...
    print(f"{Colors.CYAN}{Colors.BOLD}  AUBO E5 交互式坐标规划 - 增强版{Colors.RESET}")
    ...
    print(f"  {Colors.GREEN}Ctrl+K{Colors.RESET}                 取消当前指令（在确认阶段）")
...
def print_joint_status(move_group):
    ...
    joint_limits = [
        (-3.05, 3.05),  # shoulder: ±175°
        (-3.05, 3.05),  # upperArm: ±175°
        (-3.05, 3.05),  # foreArm: ±175°
        (-3.05, 3.05),  # wrist1: ±175°
        (-3.05, 3.05),  # wrist2: ±175°
        (-6.28, 6.28),  # wrist3: ±360°
    ]
...
def print_error_analysis(mg, trajectory, x, y, z):
    ...
    if max_error > 0.1:
        print(f"  {Colors.YELLOW}•{Colors.RESET} 关节误差较大，建议放宽收敛容差到 0.15 rad")
    if cart_error < 0.005 and max_error > 0.05:
        print(f"  {Colors.YELLOW}•{Colors.RESET} 笛卡尔误差小但关节误差大，可能是 IK 解的问题")
    if max_error_idx in [2, 3]:
        print(f"  {Colors.YELLOW}•{Colors.RESET} 关节 {max_error_idx + 1} 误差最大，检查 Unity 中该关节的 PD 参数")
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if parsed is None:
            ...
            print(f"{Colors.GRAY}示例: 0.4 0.0 0.5  或  0.4 0.0 0.5 0 1.57 0{Colors.RESET}\n")
            continue
        ...
        try:
            confirm = input(f"{Colors.GRAY}按 Enter 规划并执行，Ctrl+K 取消>{Colors.RESET} ")
            # 检查是否输入了 Ctrl+K (ASCII 11)
            if '\x0b' in confirm:
        ...
        print(f"{Colors.GREEN}✓ 规划成功{Colors.RESET}")
        ...
        try:
            ...
            if success:
                print(f"{Colors.GREEN}{Colors.BOLD}✓ 执行成功{Colors.RESET} {Colors.GRAY}({duration:.2f}秒){Colors.RESET}\n")
                stats.record_success(duration)
            else:
                print(f"{Colors.RED}{Colors.BOLD}✗ 执行失败{Colors.RESET} {Colors.GRAY}({duration:.2f}秒){Colors.RESET}")
